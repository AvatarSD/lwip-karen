...
err_t websocket_recv(void* arg, struct tcp_pcb* tpcb, struct pbuf* p, err_t err) {
    ...
    if (web_socket_open == 0) {
        ...
        if (sec_websocket_position_start) {
            ...
            memcpy(sec_and_uid_websockeet, sec_websocket_position_start,
                   sec_websocket_position_end - sec_websocket_position_start);
            sec_and_uid_websockeet[sec_websocket_position_end - sec_websocket_position_start] = 0;
            strcat(sec_and_uid_websockeet, WS_GUID);

            char result_sha[20];
            SHA1((unsigned char*)result_sha, (unsigned char*)sec_and_uid_websockeet,
        ...
        switch (opcode) {
            ...
            case 0x02:
                if (data_len > 6) {
                    ...
                    for (int i = 0; i < data_len; i++) tcp_rec[i + 6] ^= tcp_rec[2 + i % 4];
...
err_t http_recv(void* arg, struct tcp_pcb* tpcb, struct pbuf* p, err_t err) {
    ...
    char answer_html[] =
        ...
        "            var message = event.data; // Message from the server\n"
        ...
        "            if (event.key === 'Enter') {\n"
        "                ws.send(this.value); // Send the input value to the server\n"
